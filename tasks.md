...
## Phase 5: 配列 + 文字列 + 簡易 malloc — ✅ DONE

### 配列
- [x] 配列宣言のパース: `int arr[10];`
- [x] 配列初期化のパース: `int arr[] = {1, 2, 3};`（任意。なくてもよい）
...
### 簡易 malloc
- [x] バンプアロケータの実装（Wasm global でヒープポインタ管理）
- [x] `memory.grow` による動的メモリ拡張（必要に応じて）
- [x] free は実装しない（バンプアロケータなので）

### Phase 5 統合テスト
- [x] 配列の合計: `sum(int* arr, int len)` テスト
- [x] 文字列リテラルのアドレスが正しい（data section 内を指している）
- [x] 動的配列: malloc で確保した領域への読み書き
...
**完了日時**: 2026-03-20
**特記事項**: 全470テスト通過。統合テスト3件（配列合計 sum(arr,5)→15、文字列リテラル重複排除→同一アドレス、malloc動的配列→60）全て wasmtime で検証済み。  

---

## Phase 6: 仕上げ + 評価レポート — ✅ DONE

### 品質改善
- [x] エラーメッセージに行番号・カラム番号を含める
- [x] 未定義変数・未定義関数の参照時に分かりやすいエラー
- [x] 型ミスマッチ時のエラーメッセージ改善
- [x] エッジケースの修正（テスト追加含む）

### ドキュメント
- [x] README.md の作成（ビルド方法、使い方、サポートする C サブセット）
- [x] アーキテクチャ図（コンパイルパイプライン）

### 開発体験評価レポート
- [x] 仮説 1 の評価: MoonBit のコンパイル速度がイテレーション効率に与えた影響
...
**完了日時**: 2026-03-20
**特記事項**: 全524テスト通過。評価レポート完成（仮説1〜4の評価、tips と落とし穴、総合所感と今後の展望）。全6フェーズ完了。  
